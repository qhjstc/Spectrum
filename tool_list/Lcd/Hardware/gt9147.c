...
//gt9147的初始化
//IO口的对应
//PH7  T_PEN(CT_INT)中断输出信号
//PI8  T_CS(CT_RST)复位信号
...
u8 GT9147_Init(void)
{
    ...
    __HAL_RCC_GPIOD_CLK_ENABLE();			// T_PEN引脚
	...
	delay_ms(10);
 	GT_RST(1);				//释放复位		    
	delay_ms(10);
    GPIO_Initure.Pin=T_PEN_Pin;            // T_PEN引脚
    GPIO_Initure.Mode=GPIO_MODE_INPUT;      //输入
    GPIO_Initure.Pull=GPIO_NOPULL;          //不带上下拉，浮空输入
    GPIO_Initure.Speed=GPIO_SPEED_FREQ_VERY_HIGH;     //高速
    HAL_GPIO_Init(T_PEN_GPIO_Port,&GPIO_Initure);     //初始化
	delay_ms(100);  
