...
//内存池(32字节对齐)
uint8_t mem1base[MEM1_MAX_SIZE] __attribute__((section(".sram")));		    										//内部SRAM内存池
//uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(0XC01F4000))) __attribute__((align(64)));					//外部SDRAM内存池,前面2M给LTDC用了(1280*800*2)
//uint8_t mem3base[MEM3_MAX_SIZE] __attribute__((at(0X20000000))) __attribute__((align(64)));					//内部DTCM内存池
//内存管理表
